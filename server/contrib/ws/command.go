...
const (
	...
	CmdLeaveTable CmdType = "leave_table" // 主动离桌（仅限手牌间隙）；无载荷
	CmdDisconnect CmdType = "disconnect"  // 客户端主动断开（优雅离线）；无载荷
	CmdReady      CmdType = "ready"       // 结算画面点击"准备下一局"；无载荷
	CmdAddChips   CmdType = "add_chips"   // AddChipsCmd 补充桌面筹码；
)
...
type AddChipsCmd struct {
	Amount int64 `json:"amount"` // 补充金额，需大于 0 且不超过账户余额
}
